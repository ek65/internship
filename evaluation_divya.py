...
def iou(prediction, gt):
    ...
    x1_c, y1_c, w1, h1 = prediction
    x2_c, y2_c, w2, h2 = gt

    area_1 = w1*h1
    area_2 = w2*h2

    area_cap = get_area_cap((x1_c, y1_c, w1, h1), (x2_c, y2_c, w2, h2))
    # print("area_cap : ")
    # print(area_cap)
...
def read_image_label(filename):
    '''Read image names from image set'''
    labels = read_label(PREFIX_LABELS + filename + '.txt')
    label_boxes = None
...
def evaluate(gt, prediction, iou_thresh, iou2_thresh):
    ...
    detected_gt_boxes = [False]*len(gt)
    ...
    for pred in prediction:
        ...
        if not detect: # Enforcing Criteria #2
            # print("no matching ground_truth box exist to a prediction label:")
            # print(pred)
            return False

    # Enforcing Criteria #1
    for detection in detected_gt_boxes:
        if detection != True:
            return False
...
def evaluate_set(net, image_set, verbose=True):
    ...
    for file in images:
        if not os.path.exists(PREFIX_IMAGES + file + '.png'):
            continue
        gt_labels = read_image_label(file)
        predict_labels = predict(net, file)

        pred = []
        for p in predict_labels:
            pred.append(p[2:])

        correct = evaluate(gt_labels, pred, IOU_THRESH, IOU2_THRESH)
